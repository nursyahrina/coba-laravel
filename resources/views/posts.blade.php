...
<div class="container">
    <div class="row">
        ...
        <div class="col-md-4">
            <div class="card mb-3">
                <div class="fs-6 badge rounded-pill position-absolute ms-3 mt-3" style="background-color: rgba(0, 0, 0, 0.7)"><a href="/categories/{{ $post->category->slug }}" class="text-white">{{ $post->category->name }}</a></div>
                <img src="https://source.unsplash.com/500x400?{{ $post->category->name }}" class="card-img-top" alt="{{ $post->category->name }}">
                <div class="card-body">
                  <h5 class="card-title">{{ $post->title }}</h5>
                  <p class="card-text"><small class="text-muted">by <a class="fw-bold text-black" href="/authors/{{ $post->author->username }}">{{ $post->author->name }}</a> ({{ $post->created_at->diffForHumans() }})</small></p>
